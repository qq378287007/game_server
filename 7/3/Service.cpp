...
void Service::OnMsg(shared_ptr<BaseMsg> msg)
{
    ...
    lua_getglobal(luaState, "OnMsg"); // 调用Lua函数
    lua_pushinteger(luaState, msg->from);
    lua_pushinteger(luaState, msg->to);
    lua_pushstring(luaState, msg->buff.data());
    int isok = lua_pcall(luaState, 3, 0, 0);
    if (isok != 0) // 成功返回值为0，否则代表失败。
        cout << "call lua OnInit fail " << lua_tostring(luaState, -1) << endl;
}
// 退出服务时触发
